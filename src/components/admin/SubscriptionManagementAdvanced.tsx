...
import { Search, Package, Plus, Calendar, DollarSign, X, Check } from 'lucide-react';
...
export function SubscriptionManagementAdvanced() {
  ...
  const [filterStatus, setFilterStatus] = useState<'all' | 'active' | 'trial' | 'expired' | 'cancelled' | 'frozen'>('all');
  ...
  const handleApproveSubscription = async (subscriptionId: string) => {
    if (!confirm('Bu aboneliği onaylamak istediğinizden emin misiniz?')) return;
    ...
    try {
      ...
      await loadSubscriptions();
      alert('Abonelik onaylandı ve aktif hale getirildi!');
    } catch (error) {
      alert('Hata: ' + error);
    }
  };
        ...
        {filteredSubscriptions.map((subscription) => (
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              {/* ✅ YENİ: Pending subscription için onay/red butonları */}
              {subscription.status === 'pending' && (
                <div className="flex gap-2">
                  <button
                    onClick={() => handleApproveSubscription(subscription.id)}
                    className="flex-1 px-3 py-2 bg-green-500/20 hover:bg-green-500/30 text-green-300 rounded-xl text-sm font-medium transition-colors flex items-center justify-center gap-2"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 px-3 py-2 bg-red-500/20 hover:bg-red-500/30 text-red-300 rounded-xl text-sm font-medium transition-colors flex items-center justify-center gap-2"
                  >
                    <X className="w-4 h-4" />
                    Reddet
                  </button>
              ...
              {/* Mevcut butonlar - sadece active/frozen subscription'lar için */}
              {subscription.status !== 'pending' && (
                <>
                  <div className="flex gap-2">
                    <button
